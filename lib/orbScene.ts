...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  const chromaticShader = {
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Push towards neon green tone
        gl_FragColor.rgb = mix(gl_FragColor.rgb, gl_FragColor.rgb * vec3(0.8, 1.2, 0.8), 0.3);
      }
    `,
  };
  ...
  // ——— COLORS ———
  const C_BRIGHT = 0x39ff14;
  const C_MID = 0x00cc33;
  const C_DIM = 0x008822;
  const C_FAINT = 0x004411;
  const C_HOT = 0xa3ff94;
  ...
  function makeTextSprite(text: string, size = 0.08) {
    ...
    ctx.fillStyle = `rgba(${(20 + Math.random() * 60) | 0}, 255, ${(20 + Math.random() * 60) | 0}, ${alpha})`;
  ...
  g.addColorStop(0, "rgba(57,255,20,1)");
  g.addColorStop(0.2, "rgba(0,200,50,0.6)");
  g.addColorStop(0.5, "rgba(0,120,30,0.15)");
  g.addColorStop(1, "rgba(0,50,10,0)");
  dCtx.fillStyle = g;
